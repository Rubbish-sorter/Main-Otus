...
Loopback 1 здесь заменяет WAN, допущение в том, что в этой сети могут использоваться любые протоколы. В реальной сети стоит оставить за периметром только Web и используемые протоколы. Остальное запретить. Внутри сети опять же только используемые протоколы в нужных направлениях. Межсетевой экран лучше подходит для этих задач особенно на границе периметра сети.  
#### Шаг 2. Разработка и применение расширенных списков доступа, которые будут соответствовать требованиям политики безопасности.  

Cкрипт для **R1** из привилегированного режима

conf t  
ip access ext Sales  
deny tcp 10.40.0.0 0.0.0.255 10.20.0.0 0.0.0.255 eq 22  
deny tcp 10.40.0.0 0.0.0.255 any eq 80   
deny tcp 10.40.0.0 0.0.0.255 any eq 433  
permit ip any any   
exit
!
ip acces ext Operations  
...
R1(config-ext-nacl)#deny tcp 10.40.0.0 0.0.0.255 any eq 433  
R1(config-ext-nacl)#permit ip any any  
R1(config-ext-nacl)#exit  
R1(config)#!  
R1(config)#ip acces ext Operations  
...
HTTP(S) не работает в Packet Tracer  

В остальном SSH соединение не устанавливается.  
